 ...
 */
public class VersionAndUpdateTest {
    private static final Logger logger = LoggerFactory.getLogger(VersionAndUpdateTest.class);
    ...
    public void testVersionOnConcurrentUpdating() throws Exception {
        ...
        String colName = testData.getColumnsNames(tableName)[0];
        String colVal = testData.getRandomValueFromColumn(tableName, 0, dataCount);
        sql = String.format("UPDATE %s SET %s=\"%s\" WHERE row_id = 2", tableName, colName, colVal);
